...
import { isFormValid, isValidEmail, isValidPassword } from '../../utils/validation'; // Importar funciones
...
const LoginScreen = () => {
  ...
  const handleLogin = async () => {
    if (!isFormValid(email, password)) return; // Usar función importada
    setLoading(true);
    try {
  ...
  const isEmailValid = isValidEmail(email); // Usar función importada
  const isPasswordValid = isValidPassword(password); // Usar función importada
      ...
      >
        ...
        <View style={[styles.formContainer, { backgroundColor: cardBackgroundColor }]}>
          ...
          <View style={[styles.inputContainer, { backgroundColor: inputBackgroundColor, borderColor: colorScheme === 'dark' ? '#333' : '#e0e0e0' }]}>
            ...
            />
            <TouchableOpacity testID="toggle-password-icon" onPress={() => setShowPassword(!showPassword)}>
              <Ionicons
          ...
          <TouchableOpacity
            testID="login-button"
            style={[styles.loginButton, !isFormValid(email, password) && styles.loginButtonDisabled, { backgroundColor: isFormValid(email, password) ? loginButtonColor : loginButtonDisabledColor }]}
            onPress={handleLogin}
